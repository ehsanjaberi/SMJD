;(function ( $, window, document, undefined ) {
  ...
  ComboTree.prototype.createFilterHTMLForMultiSelect = function (){
    return '<input id="' + this.comboTreeId + 'MultiFilter" type="text" class="multiplesFilter" placeholder="جستجو"/>';
  }

  ComboTree.prototype.createSourceSubItemsHTML = function (subItems, parentId) {
    var subItemsHtml = '<UL id="' + this.comboTreeId + 'ComboTreeSourceUl' + (parentId ? parentId : 'main' ) + '" style="' + ((this.options.collapse && parentId) ? 'display:none;' : '')  + 'text-align: right">';
  ...
  ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
    ...
    if (this.options.isMultiple)
      itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass + '">' + (!selectableLastNode && isSelectable ? '<input type="checkbox" value='+ sourceItem.id +' name="Per[]" />' : '') + sourceItem.title + '</span>';
    else
  ...
  ComboTree.prototype.singleItemClick = function (ctItem) {
    if ($(ctItem).data("selectable") == true) {
      this._selectedItem = {
        id: $(ctItem).attr("data-id"),
        title: $(ctItem).text(),
        data: this.options.source[$(ctItem).attr("data-id")-1]
      };
